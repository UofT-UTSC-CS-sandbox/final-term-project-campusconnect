  ...
  // handles change for form input fields
    const handleChange = (selectedOptions, actionMeta) => {
      if (actionMeta?.name === "verifiedCourses") {
        setUpdatedProfile((prevState) => ({
          ...prevState,
          verifiedCourses: selectedOptions ? selectedOptions.map(option => option.value) : [],
        }));
      } 
      ...
      const handleSubmit = async (event) => {
        ...
        if (updatedProfile.verifiedCourses.length != 0 && updatedProfile.verifiedCourses.some(element => !currentProfile.verifiedCourses.includes(element))){
          if (file == null){ //transcript has not been uploaded
        ...
        if (pass == 1){
          ...
          axios.post('http://localhost:3001/updateTutorInfo', { email : email, description: updatedProfile.description, rate: updatedProfile.rate, verifiedCourses: newCourses })
          .then(response => {
      ...
      useEffect(() => {
        ...
        (async () => {
          try {
            ...
            await axios.get('http://localhost:3001/getTutorByEmail', { params: { email: email } }).then (response => {
      ...
      let i = 0;
        for (let j = 0; j < currentProfile?.verifiedCourses.length; j++){
          array[j] = {label: currentProfile?.verifiedCourses[i], value: currentProfile?.verifiedCourses[i]}
          i++;
        }
    ...
    if (currentProfile){
      return (
        ...
        <div>
          <div  style={{ display: 'flex', flexDirection: 'column',  width: '34%', alignItems: 'center' }}>
          <h1 style={{fontSize: '40px'}}>Edit Your Profile</h1>
          <br></br>
          <br></br>
          {/* print out name and image */}
          <h2 style={{ fontSize: '25px' }}>{name}</h2>
          <img src={image} width="300" height="300" style={{ marginTop: '10px' }} />
          </div>
         <div className="etp-form-wrapper" style={{display: 'flex', marginLeft: '500px',
        }} >
        ...
        />
          ...
          <input ref={rateInputRef} name="rate" required={true} className="etp-input" onChange={handleChange} />
          </div>
          <div className="etp-form-group">
          <h2 className="etp-description-text">Additional information:</h2>
          <input ref={descriptionInputRef} name="description" className="etp-input" required={true} onChange={handleChange} />
          </div>
          
          <div className="etp-form-group">
          <h2 className="etp-transcript-text">Upload your latest transcript:</h2>
          <input id='transcript' type="file" accept="application/pdf" className="etp-upload-transcript-button" onChange={getFile} />
          </div>
          <div className="etp-form-group">
          <button type="submit" className="etp-submit-button" >Save Changes</button>
          </div>
          </form>
